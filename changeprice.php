<?php
  ...
  $cloth_id = $cloth_name = $cloth_price = "";
  $cloth_id_err = $cloth_name_err = $cloth_price_err = "";
  //change price will ask user to enter the name of the cloth and the cloth id
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        {
            $cloth_name_err = "Please enter a cloth id.";
        }
        else
        {
          //select the cloth name on the database 
            ...
            if($stmt = mysqli_prepare($link, $sql))
            {
                ...
                if(mysqli_stmt_execute($stmt))
                {
                    ...
                    {
                      //if user enter the incorrect cloth name then it will give an error
                        $cloth_name_err = "Doesnt exists.";
                    }
                } 
                else
                {
        ...
        if(empty($cloth_name_err) && empty($cloth_id_err) && empty($cloth_price_err))
        {
          //code will update the cloth base on the new price
            $sql = "UPDATE clothes SET price=? WHERE (cloth_ID = ? && cloth_name = ?)";
            if($stmt = mysqli_prepare($link, $sql))
            {
                mysqli_stmt_bind_param($stmt, 'sss', $cloth_price, $cloth_id, $cloth_name);
...
<div class="w3-main" style="margin-left:250px">
  ...
  <div class="w3-container w3-center" id="jeans">
    <p><b><font size="6">Change Price</font></b></p>
</div>
