<?php
  ...
  $cloth_id = $cloth_name = $cloth_qty = $curret_cloth_qty = "";
  $cloth_id_err = $cloth_name_err = $cloth_qty_err = "";
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
      //same ass the change price, allow user to enter the cloth name and the cloth id
        ...
        {
          //then select cloth name and stock qty in the database
            $sql = "SELECT cloth_name, stock_qty FROM clothes WHERE cloth_name = '".$_POST["cloth_name"]."' && cloth_ID = '".$_POST["cloth_id"]."'";
            if($result = mysqli_query($link, $sql))
            {
                ...
                if($rowcount == 1)
                {
                        ...
                        $curret_cloth_qty = $getArray["stock_qty"];
        ...
        if(empty($cloth_name_err) && empty($cloth_id_err) && empty($cloth_qty_err))
        {
          //then update the stock equity
            $sql = "UPDATE clothes SET stock_qty=? WHERE (cloth_ID = ? && cloth_name = ?)";
            if($stmt = mysqli_prepare($link, $sql))
            {
                mysqli_stmt_bind_param($stmt, 'sss', $cloth_qty, $cloth_id, $cloth_name);
                $cloth_qty = (trim($_POST["cloth_qty"]) + $curret_cloth_qty);
                $cloth_name = trim($_POST["cloth_name"]);
                $cloth_id = trim($_POST["cloth_id"]);               
                if(!mysqli_stmt_execute($stmt))
                {
                    echo "You didn't change stock!";
                }
                else
                {
                    echo "Changed stock of the cloth!";
                }
            }
...
<body  bgcolor = "F0F0E0" class="w3-content" style="max-width:1200px">
<nav class="w3-sidebar w3-bar-block w3-white w3-collapse" style="z-index:3;width:250px" id="mySidebar">
  <div class="w3-container w3-display-container w3-padding-16">
    ...
    <h3 class="w3-wide"><font size="4"><strong>Danielle's Boutique</strong></font></h3>
	 </div>
...
<div class="w3-container w3-center" id="jeans">
    <p><b><font size="6">Add Quantity</font></b></p>
  </div>
 <div class="w3-row-padding  w3-container">
  ...
  <div class="w3-xlarge w3-padding-32">
        ...
        <p><i class="fa fa-fw fa-envelope"><font size="4"></i> DanielleBoutique@mail.com</p>
...
<div class="w3-black w3-center w3-padding-24">Danielle's Boutique</a>
</div>
<script>
function myAccFunc() {
